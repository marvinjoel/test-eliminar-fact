...
public class ContractService {
    ...
    @Transactional(readOnly = true)
    public Page<ContractListResponse> getAllContracts(String status, String search, int page, int size) {

        String finalStatus = (status != null && !status.trim().isEmpty() && !status.equalsIgnoreCase("TODOS"))
                ? status.toUpperCase() : null;

        // 💡 CAMBIO CLAVE AQUÍ: Le agregamos los % y lo pasamos a minúsculas en Java
        String finalSearch = (search != null && !search.trim().isEmpty())
                ? "%" + search.toLowerCase() + "%" : null;

        // 2. Llamada a la BD
        Page<ContractEntity> contractsPage = contractRepository.findContractsWithFilters(
                finalStatus,
                finalSearch,
                PageRequest.of(page, size)
        );

        // 2. Mapeo original (tu código intacto)
        return contractsPage.map(c -> {
            ...
            BigDecimal initialPaymentAmount = BigDecimal.ZERO;
            String finalMethod = "---";
            BigDecimal finalPaymentAmount = BigDecimal.ZERO;
